package br.com.megahack.releave.utils;
...
public class BRUtil {
	...
	public static boolean isValidCPF(String ssn) {
		if(!StringUtils.hasText(ssn)) return false;
		
		ssn = ssn.replaceAll("\\D", "");
		
		if ((ssn.length() != 11) || ssn.matches(ssn.charAt(0) + "{11}"))
			return false;
	...
	public static boolean isValidCNPJ(String tin) {
		if(!StringUtils.hasText(tin)) return false;
		
		tin = tin.replaceAll("\\D", "");
		
		if ((tin.length() != 14) || tin.matches(tin.charAt(0) + "{14}"))
			return false;
